...
if check_ip_and_get_info "$IP_VPS"; then
    ...
    echo -e "➥ Hubungi admin ${CYAN}「 ✦ @SynchVPN_bot ✦ 」${NC}"
    exit 1
fi
...
show_footer() {
    ...
    echo -e " ${COLOR1}║                  ${WH}• SYNCH STORE •                ${COLOR1}║${NC}"
    echo -e " ${COLOR1}╚════════════════════════════════════════════════════╝${NC}"
}
...
function skt-domain() {
    ...
    install_slowdns() {
        local script_url="https://raw.githubusercontent.com/G-Force-X/nextvpn/refs/heads/main/SKT-DnsTT/installsl.sh"
...
10 |10) clear ; wget -q https://raw.githubusercontent.com/G-Force-X/nextvpn/refs/heads/main/rebuildpepesmenu && bash rebuildpepesmenu ;; 
00 |0) clear ; menu ;; 
